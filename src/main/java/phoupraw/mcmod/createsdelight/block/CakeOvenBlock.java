...
public class CakeOvenBlock extends Block implements IBE<CakeOvenBlockEntity>, IWrenchable {
    ...
    public BlockState rotate(BlockState state, BlockRotation rotation) {
        return state.with(FACING, rotate2(state.get(FACING), rotation));
    }
    ...
    public static RailShape rotate2(RailShape facing, BlockRotation rotation) {
        return RAIL_SHAPE_MAP.inverse().get(RAIL_SHAPE_MAP.get(facing).stream().map(rotation::rotate).collect(Collectors.toSet()));
    }

    public static RailShape mirror2(RailShape facing, BlockMirror mirror) {
    ...
    public BlockState mirror(BlockState state, BlockMirror mirror) {
        return state.with(FACING, mirror2(state.get(FACING), mirror));
    }
